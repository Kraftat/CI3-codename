...
<section class="panel">
	<div class="tabs-custom">
		<ul class="nav nav-tabs">
			<li class="active">
				<a href="#productlist" data-toggle="tab"><i class="far fa-list-ul"></i> <?php echo translate('purchase') . ' ' . translate('list'); ?></a>
			</li>
...
<?php endif; ?>
							...
							<?php if (get_permission('purchase_payment', 'is_add')): ?>
								<a href="<?php echo base_url('inventory/purchase_bill/' . $row['id']); ?>" class="btn btn-circle icon btn-default" data-toggle="tooltip" data-original-title="<?php echo translate('bill_view'); ?>"> <i class="far fa-credit-card"></i></a>
							<?php endif; ?>
							<?php if (get_permission('product_purchase', 'is_edit')): 
								if ($row['purchase_status'] != 2) { ?>
									<button class="btn btn-circle icon btn-default" data-toggle="tooltip" data-original-title="<?php echo translate('make_received'); ?>" onclick="confirmStock('<?php echo $row['id'] ?>')"><i class="far fa-plus-square"></i></button>
								<?php } ?>
								<a href="<?php echo base_url('inventory/purchase_edit/' . $row['id']); ?>" class="btn btn-circle icon btn-default" data-toggle="tooltip" data-original-title="<?php echo translate('edit'); ?>"><i class="far fa-pen-nib"></i></a>
							<?php endif; if (get_permission('product_purchase', 'is_delete')): ?>
				...
				<?php echo form_open('inventory/purchase_save', array('id' => 'frmSubmit')); ?>
					...
					<?php endif; ?>
						<div class="form-group">
							...
							<div class="col-md-6">
								<?php
									$supplierlist = $this->appLib->getSelectByBranch('product_supplier', $branch_id);
						...
						<div class="form-group">
							...
							<div class="col-md-6">
								<?php
									$storelist = $this->appLib->getSelectByBranch('product_store', $branch_id);
						...
						<div class="form-group">
							...
							<div class="col-md-6">
								<input type="text" class="form-control" name="bill_no" value="<?php echo $this->appLib->get_bill_no('purchase_bill'); ?>" id="bill_no" />
								<span class="error"></span>
							</div>
						</div>
						<div class="form-group">
					...
					<div id="purchaseItems" style="<?php echo (empty($branch_id) ? 'display: none;' : ''); ?>">
						...
						<div class="table-responsive">
							<table class="table table-bordered table-hover mt-md" id="tableID">
								...
								<tfoot>
									<tr>
										<td colspan="1"><button type="button" class="btn btn-default" onclick="addRows()"> <i class="far fa-plus-circle"></i> <?php echo translate('add_rows'); ?></button></td>
					...
					<footer class="panel-footer">
						<div class="row">
							<div class="col-md-offset-9 col-md-3">
								<button type="submit" name="purchase" id="savebtn" value="1" class="btn btn-default btn-block" data-loading-text="<i class='far fa-spinner fa-spin'></i> Processing">
...
<?php endforeach; ?>
		...
		html_row += '<button type="button" class="btn btn-danger" onclick="deleteRow(' + value + ')" style="float: right; max-width: 30%"><i class="far fa-times"></i> </button>';
		html_row += '</td>';
		html_row += '</tr>';
		return html_row;
	}
