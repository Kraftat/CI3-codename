...
$user = $applicationModel->getUserNameByRoleID($result->role_id, $result->user_id);
?>
	<header class="panel-heading">
		<h4 class="panel-title"><i class="far fa-list-ol"></i> <?=translate('issue') . " " . translate('details')?></h4>
	</header>
					...
					<div class="panel-body">
						<div class="table-responsive">
							<table class="table table-bordered">
								...
								<?php
								$count = 1;
								$builder->select('product_issues_details.*,product.name as product,product_category.name as category,sales_unit_id');
								$this->db->from('product_issues_details');
								$builder->join('product', 'product.id = product_issues_details.product_id', 'inner');
								...
								$allowances = $builder->get()->result_array();
								if(!empty($allowances)){
	...
	<footer class="panel-footer">
		...
		<?php if (get_permission('product_issue', 'is_add')):
			if ($result->status == 0) {
				?>
			<button class="btn btn-primary mr-xs btn-return" data-loading-text="<i class='far fa-spinner fa-spin'></i> Processing">
