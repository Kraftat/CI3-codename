...
<?php if($total_paid > 1 && get_permission('purchase_payment', 'is_view')):?>
			<li class="<?php echo ($active_tab == 2 ? 'active' : ''); ?>">
				<a href="#payment_history" data-toggle="tab"><i class="far fa-dollar-sign"></i> <?php echo translate('payment_history'); ?></a>
			</li>
							...
							<div class="row">
								<div class="col-xs-6">
									<div class="bill-data">
										...
										<address>
											<?php
											$stuDetails = $applicationModel->getUserNameByRoleID($billdata['role_id'], $billdata['user_id']);
...
<?php } ?>
					...
					<div class="text-right mr-lg hidden-print">
						<button class="btn btn-default ml-sm" onClick="fn_printElem('invoice_print')"><i class="far fa-print"></i> <?php echo translate('print'); ?></button>
					</div>
				</div>
			</div>
<?php if($total_paid > 1 && get_permission('purchase_payment', 'is_view')): ?>
			<div class="tab-pane <?php echo ($active_tab == 2 ? 'active' : ''); ?>" id="payment_history">
				<div id="payment_print">
					...
					<div class="text-right mr-lg hidden-print">
						<button class="btn btn-default" onClick="fn_printElem('payment_print')"><i class="far fa-print"></i> <?php echo translate('print'); ?></button>
					</div>
				</div>
			</div>
<?php endif; ?>
					...
					<footer class="panel-footer">
						<div class="row">
							<div class="col-md-offset-3 col-md-3">
								<button class="btn btn-default" type="submit" data-loading-text="<i class='far fa-spinner fa-spin'></i> Processing"><?php echo translate('payment'); ?></button>
							</div>
						</div>
					</footer>
				<?= form_close(); ?>
