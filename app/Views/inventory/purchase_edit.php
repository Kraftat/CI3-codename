<section class="panel">
	<div class="tabs-custom">
		<ul class="nav nav-tabs">
			<li>
				<a href="<?php echo base_url('inventory/purchase'); ?>"><i class="far fa-list-ul"></i> <?php echo translate('purchase') . ' ' . translate('list'); ?></a>
			</li>
			<li class="active">
					...
					<div class="table-responsive">
						<table class="table table-bordered mt-md nowrap" id="tableID">
							...
							<tbody>
								...
								$bill_details = $builder->getWhere('purchase_bill_details', array('purchase_bill_id' => $purchaselist['id']))->result();
								if(count($bill_details)){
								foreach ($bill_details as $key => $product):


									$builder->select('product_unit.name');
									$this->db->from('product');
									$builder->join('product_unit','product_unit.id = product.purchase_unit_id', 'inner');
									$this->db->where('product.id', $product->product_id);
									$unitName = $builder->get()->row()->name;
					...
					<footer class="panel-footer">
						<div class="row">
							<div class="col-md-offset-9 col-md-3">
								<button type="submit" name="update" id="savebtn" value="1" class="btn btn-default btn-block" data-loading-text="<i class='far fa-spinner fa-spin'></i> Processing">
