<section class="panel">
	<div class="tabs-custom">
		<ul class="nav nav-tabs">
			<li>
				<a href="<?=base_url('award')?>"><i class="far fa-list-ul"></i> <?=translate('award') . ' ' . translate('list')?></a>
			</li>
			<li class="active">
				...
				<div class="form-group" id="classDiv" style="<?=($award['role_id'] != 7 ? 'display: none;' : '');?>" >
					...
					<div class="col-md-6">
						<?php
							...
							if($award['role_id'] == 7){
								$class_id = $builder->getWhere('enroll', array('student_id' => $award['user_id']))->row()->class_id;
								$arrayClass = $this->appLib->getClass($award['branch_id']);
							}
				...
				<div class="form-group">
					...
					<div class="col-md-6">
						<?php
							...
							if($award['role_id'] != 7){
								$arrayUser = $this->appLib->getStaffList($award['branch_id'], $award['role_id']);
							} else {
								...
								foreach ($query_enroll as $enroll){
									$s = $builder->select('id,CONCAT(first_name," ",last_name) as full_name')->where(array('id' => $enroll['student_id']))->get('student')->row();
