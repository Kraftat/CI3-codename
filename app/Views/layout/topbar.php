<header class="header">
	<div class="logo-env">
		<a href="<?php echo base_url('dashboard');?>" class="logo">
			<img src="<?=$applicationModel->getBranchImage(get_loggedin_branch_id(), 'logo-small')?>" height="40">
		</a>
		...
		<ul class="header-menu">
			...
			<li>
				<div class="header-menu-icon sidebar-toggle" data-toggle-class="sidebar-left-collapsed" data-target="html" data-fire-event="sidebar-left-toggle">
					<i class="far fa-bars" aria-label="Toggle sidebar"></i>
				</div>
			</li>
			<!-- full screen button -->
			<li>
				...
				<div class="dropdown-menu header-menubox qk-menu">
					<div class="qk-menu-p">
						...
						<?php if(get_permission('student', 'is_add')){ ?>
							<a href="<?php echo base_url('student/add');?>"><i class="far fa-users"></i> <?php echo translate('student_admission');?></a>
						<?php } if(get_permission('salary_payment', 'is_add')) { ?>
							<a href="<?php echo base_url('payroll'); ?>"><i class="far fa-donate"></i> <?php echo translate('salary_payment');?></a>
						<?php } if(get_permission('leave_manage', 'is_view')) { ?>
							<a href="<?php echo base_url('leave');?>"><i class="far fa-plane"></i> <?php echo translate('leave_application');?></a>
						<?php } if(get_permission('live_class', 'is_view')) { ?>
							<a href="<?php echo base_url('live_class');?>"><i class="far fa-video"></i> <?php echo translate('live_class_rooms');?></a>
						<?php } if(get_permission('due_invoice', 'is_view')) { ?>
							<a href="<?php echo base_url('fees/due_invoice');?>"><i class="far fa-hand-holding-usd"></i> <?php echo translate('due_fees_invoice');?></a>
...
if (is_student_loggedin()):
	$builder->select('enroll.id,class.name as class_name,section.name as section_name');
	$this->db->from('enroll');
	$builder->join('class', 'class.id = enroll.class_id', 'inner');
	$builder->join('section', 'section.id = enroll.section_id', 'inner');
			...
			<li>
				...
				<div class="dropdown-menu header-menubox mh-oh">
					<div class="notification-title">
						<i class="fa-regular fa-retweet"></i> <?php echo translate('switch_class');?>
					</div>
...
foreach ($multiClass as $key => $class):
	...
	<li>
		<a href="<?php echo base_url('userrole/switchClass/' . $class->id);?>">
			<?php echo $class->class_name . " (" . $class->section_name . ")";  ?>  <?php echo $class->id == $currentClass ? '<i class="far fa-check"></i>' : ''; ?>
		</a>
	</li>
<?php endforeach;?>
								</ul>
...
<?php  } 
	...
	?>
		...
		if (is_superadmin_loggedin()) {
			...
			$cmsRow = $builder->select('cms_active,url_alias')
					...
					<div class="content hbox pr-none">
						<div class="scrollable visible-slider dh-tf" data-plugin-scrollable>
							<div class="scrollable-content">
								<ul>
<?php
$get_session = $builder->get('schoolyear')->result();
foreach ($get_session as $session) : 
?>
	<li>
		<a href="<?php echo base_url('sessions/set_academic/' . $session->id);?>">
			<?php echo $session->school_year;?> <?php echo get_session_id() == $session->id ? '<i class="far fa-check"></i>' : ''; ?>
		</a>
	</li>
<?php endforeach;?>
								</ul>
...
$languages = $builder->select('id,lang_field,name')->where('status', 1)->get('language_list')->result();
foreach($languages as $lang) :
?>
	<li>
		<a href="<?php echo base_url('translations/set_language/' . html_escape($lang->lang_field));?>">
		<img class="ln-img" src="<?php echo $applicationModel->getLangImage($lang->id);?>" 
		alt="<?php echo $lang->lang_field;?>"> <?php echo ucfirst(html_escape($lang->name));?> <?php echo ($set_lang == $lang->lang_field ? '<i class="far fa-check"></i>' : ''); ?>
		</a>
	</li>
<?php endforeach;?>
								</ul>
			...
			<li>
				<a href="#" class="dropdown-toggle header-menu-icon" data-toggle="dropdown">
					...
					<?php 
						$unreadMessage	= $applicationModel->unread_message_alert();
			...
			<div class="dropdown-menu">
				<ul class="dropdown-user list-unstyled">
					<li class="user-p-box">
						<div class="dw-user-box">
							...
							<div class="u-text">
								...
								<!-- <a href="<?php echo base_url('authentication/logout'); ?>" class="btn btn-danger btn-xs"><i class="far fa-sign-out-alt"></i> <?php echo translate('logout');?></a> -->
							</div>
						</div>
					</li>
					<li role="separator" class="divider"></li>
					<li><a href="<?php echo base_url('profile');?>"><i class="far fa-user-shield"></i> <?php echo translate('profile');?></a></li>
					<li><a href="<?php echo base_url('profile/password');?>"><i class="far fa-mars-stroke-h"></i> <?php echo translate('reset_password');?></a></li>
					...
					<?php if(get_permission('global_settings', 'is_view')):?>
						<li role="separator" class="divider"></li>
						<li><a href="<?php echo base_url('settings/universal');?>"><i class="far fa-toolbox"></i> <?php echo translate('global_settings');?></a></li>
					<?php endif; ?>
					<?php if(get_permission('school_settings', 'is_view') && !is_superadmin_loggedin()):?>
						<li role="separator" class="divider"></li>
						<li><a href="<?php echo base_url('school_settings');?>"><i class="far fa-school"></i> <?php echo translate('school_settings');?></a></li>
					<?php endif; ?>
					<li role="separator" class="divider"></li>
					<li><a href="<?php echo base_url('authentication/logout');?>"><i class="far fa-sign-out-alt"></i> <?php echo translate('logout');?></a></li>
				</ul>
			</div>
		</div>
	</div>
