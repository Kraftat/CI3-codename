<section class="panel">
	<header class="panel-heading">
		<h4 class="panel-title"><i class="far fa-list-ul"></i> <?=translate('subject_list')?></h4>
	</header>
	<div class="panel-body">
		<table class="table table-bordered table-hover mb-none table-export">
			...
			<tbody>
				...
				$builder->select('sa.subject_id,sa.class_id,sa.teacher_id,s.name as subject_name,s.subject_code,s.subject_type,s.subject_author,t.name as teacher_name');
				$this->db->from('subject_assign as sa');
				$builder->join('subject as s','s.id = sa.subject_id', 'left');
				$builder->join('staff as t','t.id = sa.teacher_id', 'left');
				$this->db->where('sa.class_id', $stu['class_id']);
				$subjectlist = $builder->get()->result_array();
