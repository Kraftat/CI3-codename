									...
									<div class="col-md-4">
										<ul class="nav nav-stacked">
											...
											if ($row['ev_status'] == 'u' || $row['ev_status'] == '') {
												$submitStatus = $builder->select('id')->where(['student_id' => $row['student_id'], 'homework_id' => $row['id']])->get('homework_submit')->row();
										...
										<ul class="nav nav-stacked mt-md">
											...
											<li><span class="text-weight-semibold"><?=translate('documents')?></span> : <a href="<?=base_url('homework/download/' . $row['id'])?>" style="display: initial;" class="btn btn-default btn-circle icon" data-toggle="tooltip" data-original-title="<?=translate('download')?>"><i class="far fa-cloud-download-alt"></i></a></li>
										</ul>
									<?php if (!empty($row['enc_name'])) { ?>
										<ul class="nav nav-stacked mt-md">
											<li><span class="text-weight-semibold"><?=translate('submitted_file')?></span> : <a href="<?=base_url('homework/download_submitted?file=' . $row['enc_name'])?>" style="display: initial;" class="btn btn-default btn-circle icon" data-toggle="tooltip" data-original-title="<?=translate('download')?>"><i class="far fa-cloud-download-alt"></i></a></li>
										</ul>
									<?php 
										}
										...
										?>
										<ul class="nav nav-stacked mt-md">
											<li><span class="text-weight-semibold"><?=translate('assignment')?></span> : <button onclick="showModal('<?php echo $row['id']?>')" class="btn btn-default btn-sm"><i class="far fa-upload"></i> <?=translate('upload')?></button></li>
										</ul>
									<?php } ?>
									</div>
								</div>
...
<div class="zoom-anim-dialog modal-block mfp-hide" id="modal">
	<section class="panel">
		<header class="panel-heading">
			<h4 class="panel-title"><i class="far fa-bars"></i> <?php echo translate('assignment'); ?></h4>
		</header>
		...
		<footer class="panel-footer">
			<div class="row">
				<div class="col-md-12 text-right">
					<button type="submit" class="btn btn-default" data-loading-text="<i class='far fa-spinner fa-spin'></i> Processing"><?php echo translate('submit'); ?></button>
