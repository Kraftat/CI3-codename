<div class="row">
	<div class="col-md-12">
		...
		<section class="panel" >
			<header class="panel-heading">
				<h4 class="panel-title"><i class="far fa-sitemap"></i> <?php echo translate('school') . " " . translate('list');?></h4>
			</header>
					...
					<tbody>
						...
						<tr>
							...
							<?php if ($row->payment_data == 'olp') { ?>
								<button class="btn btn-circle icon btn-default" data-toggle="tooltip" data-original-title="<?=translate('offline_payments')?>" data-loading-text="<i class='far fa-spinner fa-spin'></i>" onclick="getApprovelOfflinePayments(this, '<?=$row->id?>')"><i class="far fa-credit-card"></i></button>
							<?php } if ($row->status != 1) { ?>
								...
								<?php if ($row->status == 0) { ?>
								<button  onclick="getApprovelLeaveDetails(this, '<?php echo $row->id?>')" class="btn btn-default btn-circle icon" data-loading-text="<i class='far fa-spinner fa-spin'></i>" data-toggle="tooltip" data-original-title="<?=translate('reject')?>">
									<i class="far fa-times-circle"></i>
								</button>
								<?php } 
								} ?>
								<?php echo btn_delete('saas/delete/' . $row->id);?>
								<a href="<?php echo base_url('saas_payment/index/' . $row->reference_no); ?>" class="btn btn-info btn-circle icon" data-toggle="tooltip" data-original-title="<?=translate('view')?>">
								<i class="far fa-eye"></i>
								<?php if ($row->payment_status == 1) { ?>
                    <a href="<?php echo base_url('subscription_review/' . $row->reference_no); ?>" class="btn btn-success btn-circle icon" data-toggle="tooltip" data-original-title="<?=translate('invoice')?>" target="_blank">
