<section class="panel">
	<header class="panel-heading">
		<h4 class="panel-title"><i class="fa-regular fa-building-columns"></i> <?=translate('school') . " " . translate('details')?></h4>
		<div class="panel-btn">
			<a href="<?php echo base_url('saas/school') ?>" class="btn btn-default btn-circle"><i class="fa-regular fa-arrow-left"></i> <?=translate('back')?></a>
		</div>
	</header>
	<div class="panel-body">
			...
			<table class="table table-striped table-condensed mb-lg mt-lg">
				<tbody>
					...
					<tr>
						...
						<td><?php echo empty($school->country) ? 'N/A' : $school->country; ?></td>
					</tr>
					<tr>
		...
		<div class="row">
				    ...
				    <div class="panel-body">
				        ...
				        <div class="mt-lg">
							...
							<table class="table table-bordered table-condensed tbr-middle table-export nowrap" width="100%" cellspacing="0">
								...
								<tbody>
									<?php
									$builder->select('sst.*,payment_types.name as payvia,saas_package.name as package_name,saas_package.free_trial');
									$this->db->from('saas_subscriptions_transactions as sst');
									$this->db->where('subscriptions_id', $school->subscriptions_id);
									$builder->join('payment_types', 'payment_types.id = sst.payment_method', 'left');
									$builder->join('saas_package', 'saas_package.id = sst.package_id', 'left');
									$this->db->order_by('sst.id', 'asc');
									$getPermissions = $builder->get()->result();
									$count = 1;
									...
									<tr>
										...
										<td>
											...
											if ($value->free_trial == 0) {
												echo $value->currency . ' ' . number_format(($value->amount - $value->discount), 2, '.', '');
											} else {
												echo translate('free_trial');
											}
											?>
										</td>
			...
			<div class="col-lg-5 col-md-12">
				<section class="panel pg-fw">
				    <div class="panel-body">
				        <h5 class="chart-title mb-xs"><i class="fa-regular fa-sliders"></i> <?=translate('modules') . " " . translate('permission')?></h5>
				        <div class="mt-lg">
							<div class="table-responsive">
								<table class="table table-bordered table-condensed tbr-middle" width="100%" cellspacing="0">
									...
									<tbody>
										<?php
										$builder->select('pm.name,isEnabled');
										$this->db->from('modules_manage');
										$builder->join('permission_modules as pm', 'pm.id = modules_manage.modules_id', 'left');
