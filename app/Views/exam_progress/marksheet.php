				...
				<?php endif; ?>
							...
							<?php
								if(!empty($branch_id)){
									...
									foreach ($exams as $row){
										$arrayExam[$row->id] = $applicationModel->exam_name_by_id($row->id);
									}
								} else {
					...
					<div class="col-md-3 mt-xs">
						<div class="form-group">
							...
							<?php
								$arraySection = $this->appLib->getSelectByBranch('marksheet_template', $branch_id);
			...
			<section class="panel appear-animation" data-appear-animation="<?= $global_config['animations']?>" data-appear-animation-delay="100">
				...
				<header class="panel-heading">
					<h4 class="panel-title">
						<i class="far fa-users"></i> <?=translate('student_list')?>
					</h4>
					<div class="panel-btn">
						...
						<button type="button" id="downloadPDF"class="btn btn-default btn-circle" data-loading-text="<i class='far fa-spinner fa-spin'></i> Processing">
					...
					<table class="table table-bordered table-hover table-condensed mb-none mt-sm mb-md" id="marksheet">
						...
						<tbody>
							...
							<tr>
								...
								<td>
									<button type="button" data-loading-text="<i class='far fa-spinner fa-spin'></i>" data-placement="top" data-toggle="tooltip" data-original-title="<?php echo translate('email') . " " . translate('marksheet') ?>" class="btn btn-default icon btn-circle" onclick="pdf_sendByemail('<?=$row['id']?>', '<?=$row['enrollID']?>', this)"><i class="fa-regular fa-envelope"></i></button>
								</td>
							</tr>
						<?php 
							endforeach; 
