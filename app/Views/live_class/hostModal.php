...
<div class="row">
	<div class="col-md-8">
		...
		<?php if (empty($row['remarks'])) { ?>
			<div class="alert alert-subl text-dark text-center"><i class="far fa-exclamation-circle"></i> No information was found</div>
		<?php } else { ?>
...
?>
		<a href="<?=base_url('live_class/zoom_meeting_start?meeting_id=' . $row['meeting_id'] . "&live_id=" . $row['id'])?>" class="btn btn-default mt-md mb-md"><i class="far fa-video"></i> <?=translate('host_live_class')?></a>
		<a href="<?=$startURL?>" class="btn btn-default mb-md">Host By Zoom APP</a>
<?php } elseif($row['live_class_method'] == 2) { ?>
	<a href="<?=base_url('live_class/bbb_meeting_start?meeting_id=' . $row['meeting_id'] . "&live_id=" . $row['id'])?>" class="btn btn-default mt-md mb-md"><i class="far fa-video"></i> <?=translate('host_live_class')?></a>
<?php } elseif($row['live_class_method'] == 3) { ?>
	<a href="<?=json_decode($row['bbb'])->join_url?>" class="btn btn-default mt-md mb-md"><i class="far fa-video"></i> <?=translate('host_live_class')?></a>
<?php } ?>
	</div>
</div>