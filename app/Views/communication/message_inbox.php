<section class="panel">
	...
	<div class="panel-body">
		<table class="table text-dark table-hover table-condensed mb-none table-export">
			...
			$messages = $builder->getWhere('message', array('reciever' => $active_user, 'trash_inbox' => 0))->result();
			...
			?>
				<tr <?php if($message->read_status == 0) { ?> class="text-weight-bold" <?php } ?>>
					...
					<td width="20%">
						...
						data-toggle="tooltip" data-original-title="Click to teach if this conversation is important"><i class="text-warning <?=($message->fav_inbox == 0 ? 'far fa-bell' : 'far fa-bell')?>"></i></a><a href="<?=base_url('communication/mailbox/read?type=inbox&id='.$message->id)?>" class="text-dark mail-subj"><?='&nbsp;&nbsp;&nbsp;'.$applicationModel->getUserNameByRoleID($senderRoleID, $senderUserID)['name']?></a>
					</td>
					<td>
						<?php echo (!empty($message->file_name) ? '<i class="far fa-paperclip"></i>' : ''); ?>
