	...
	<div class="panel-body">
		<h3 class="m-none text-weight-light">
			...
			<a data-id="<?=$message_id?>" href="javascript:;" class="mailbox-fav" data-toggle="tooltip" data-original-title="Click to teach if this conversation is important">
				<i class="text-warning <?=($status == 0 ? 'far fa-bell' : 'far fa-bell');?>"></i>
			</a>
		</h3>
		<p class="mt-lg mb-none text-md">
			<?php 
			echo 'From <strong class="text-dark">'.$applicationModel->getUserNameByRoleID($senderRoleID, $senderUserID)['name'].'</strong> To ';
			echo 'From <strong class="text-dark">'.$applicationModel->getUserNameByRoleID($recieverRoleID, $recieverUserID)['name'].'</strong> , Started On ';
...
<div class="panel">
	<div class="panel-heading">
		<h4 class="panel-title">
			<?php echo $applicationModel->getUserNameByRoleID($senderRoleID, $senderUserID)['name']; ?>
		</h4>
	</div>
	<div class="panel-body">
	...
	<?php if (!empty($message->enc_name)): ?>
		<blockquote class="warning">
			...
			<a href="<?=base_url('communication/download?type=mailbox&file='.$message->enc_name)?>" class="btn btn-default btn-sm"><i class="far fa-paper-plane"></i><span> Download</span></a>
		</blockquote>
	<?php endif; ?>
	</div>
...
<?php
$repliesResult = $builder->getWhere('message_reply', array('message_id' => $message_id))->result();
$reply_status = $builder->select('sender,reciever')->where('id', $message_id)->get('message')->row();
...
<?php if (!empty($reply->enc_name)): ?>
		<blockquote class="warning">
			<p><?=translate('attachment_file')?></p>
			<a href="<?=base_url('communication/download?type=reply&file='.$reply->enc_name)?>" class="btn btn-default btn-sm"><i class="far fa-paper-plane"></i><span> Download</span></a>
		</blockquote>
<?php endif; ?>
	</div>
	<div class="panel-footer">
			...
			<div class="row">
				<div class="col-md-offset-10 col-md-2">
					<button type="submit" class="btn btn-default btn-block" data-loading-text="<i class='far fa-spinner fa-spin'></i> Processing">
						<i class="far fa-paper-plane"></i><span> <?=translate('send')?></span>
					</button>
				</div>
			</div>
		</div>
