<section class="panel">
	...
	<div class="panel-body">
		<table class="table text-dark table-hover table-condensed mb-none table-export">
			...
			$messages = $builder->getWhere('message', array('sender' => $active_user, 'trash_sent' => 0))->result();
			...
			?>
				<tr <?=($message->reply_status == 1 ? 'class="text-weight-bold"' : '');?>>
					...
					<td width="20%">
						...
						data-original-title="Click to teach if this conversation is important"><i class="text-warning <?=($message->fav_sent == 0 ? 'far fa-bell' : 'far fa-bell');?>"></i></a><a href="<?=base_url('communication/mailbox/read?type=sent&id='.$message->id)?>" class="text-dark mail-subj"><?='&nbsp;&nbsp;&nbsp;'. $applicationModel->getUserNameByRoleID($recieverRoleID, $recieverUserID)['name']?></a>
					</td>
					<td>
