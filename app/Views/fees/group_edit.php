<section class="panel">
	<div class="tabs-custom">
		<ul class="nav nav-tabs">
			<li>
				<a href="<?=base_url('fees/group')?>"><i class="far fa-list-ul"></i> <?php echo translate('fees_group') . " " . translate('list'); ?></a>
			</li>
			<li class="active">
					...
					<div class="table-responsive">
						<table class="table table-bordered table-hover" id="tableID">
							...
							<tbody>
								...
								$result = $builder->get('fees_type')->result_array();
								foreach ($result as $key => $row) {
									$this->db->where('fee_groups_id', $group['id']);
									$this->db->where('fee_type_id', $row['id']);
									$details = $builder->get('fee_groups_details')->row_array();
									if (empty($details)) {
										$details['fee_type_id'] = '';
										$details['due_date'] = '';
										$details['amount'] = '0.00';
