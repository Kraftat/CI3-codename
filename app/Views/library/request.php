<section class="panel">
	<div class="tabs-custom">
		<ul class="nav nav-tabs">
			<li class="active">
				<a href="#list" data-toggle="tab"><i class="far fa-list-ul"></i> <?php echo translate('book_issued'); ?></a>
			</li>
			...
			<div id="list" class="tab-pane active">
				<table class="table table-bordered table-hover table-condensed mb-none tbr-top table-export">
					...
					<tbody>
						<?php
							$count = 1;
							$builder->select('bi.*,b.title,b.isbn_no,b.edition,b.author,b.publisher,c.name as category_name');
							...
							$booklist =  $builder->get()->result_array();
						...
						<tr>
							...
							<?php if (get_permission('book_request', 'is_delete')): ?>
								<button class="btn btn-danger btn-circle" <?=($status != 0 ? 'disabled' : '');?> onclick="confirm_modal('<?=base_url('library/request_delete/' . $row['id'] ) ?>')">
									<i class="far fa-trash-alt"></i> <?=translate('delete')?>
								</button>
							<?php endif; ?>
							</td>
						</tr>
					...
					<div class="form-group">
						...
						<div class="col-md-6">
							<?php
								...
								$books = $builder->select('id,title')->get_where('book', array('branch_id' => get_loggedin_branch_id()))->result();
