<section class="panel">
	<div class="tabs-custom">
		<ul class="nav nav-tabs">
			<li class="active">
				<a href="#list" data-toggle="tab"><i class="far fa-list-ul"></i> <?=translate('books_list')?></a>
			</li>
				...
				<table class="table table-bordered table-hover table-condensed mb-none tbr-top table-export">
					...
					<tbody>
						...
						<tr>
							...
							<td><img src="<?php echo $applicationModel->get_book_cover_image($row['cover']); ?>" alt="" width="70"></td>
							<td><?php echo $row['role_name'];?></td>
							<td>
							<?php 
								if ($row['role_id'] == 7) {
								 	$getStudent = $applicationModel->getStudentDetails($row['user_id']);
								 	echo $getStudent['first_name'] . " " . $getStudent['last_name'] . '<br><small> - ' .
								 	$getStudent['class_name'] . ' (' . $getStudent['section_name'] . ')</small>';
								} else {
									$getStaff = $builder->select('name,staff_id')->where('id', $row['user_id'])->get('staff')->row_array();
							...
							<td><?= $global_config['currency_symbol'] . $row['fine_amount']; ?></td>
							<td>
								<?php
								$status = $row['status'];
								if($status == 0)
							...
							<?php if (get_permission('book_manage', 'is_add')): ?>	
								...
								<a href="javascript:void(0);" class="btn btn-circle btn-default" onclick="getBookApprovelDetails('<?=$row['id']?>');">
									<i class="far fa-bars"></i> <?=translate('details')?>
								</a>
								<?php if($status == 1) { ?>
									<!-- return modal link -->
									...
									data-expiry="<?=$row['date_of_expiry']?>"  onclick="getBookReturn(this);">
										<i class="far fa-exchange-alt"></i> <?=translate('return')?>
									</a>
								<?php
								}
							endif;
			...
			<div class="tab-pane" id="issue">
				<?php echo form_open('library/bookIssued', array('class' => 'form-horizontal form-bordered frm-submit')); ?>
					...
					<div class="form-group">
						...
						<div class="col-md-6">
							<?php
								$arrayCategory = $this->appLib->getSelectByBranch('book_category', $branch_id);
						...
						<div class="col-md-6">
							<?php
								if(!empty($category_id)){
									...
									$books = $builder->getWhere('book', array('category_id' => $category_id))->result();
			        ...
			        <div class="form-group">
						...
						<div class="col-md-6">
			                <?php
			                    $role_list = $this->appLib->getRoles([1,6]);
	...
	<section class="panel">
		<header class="panel-heading">
			<h4 class="panel-title">
				<i class="far fa-exchange-alt"></i> <?=translate('return')?></h4>
			</h4>
		</header>
