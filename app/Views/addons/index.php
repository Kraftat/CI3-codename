	...
	<div class="tabs-custom">
		<ul class="nav nav-tabs">
			<li class="<?=(empty($validation_error) ? 'active' : '') ?>">
				<a href="#list" data-toggle="tab"><i class="far fa-list-ul"></i> <?=translate('addon') . " " . translate('list')?></a>
			</li>
			<li class="<?=(!empty($validation_error) ? 'active' : '') ?>">
				<a href="#create" data-toggle="tab"><i class="far fa-boxes"></i> <?=translate('install') . " " . translate('addon')?></a>
			</li>
		</ul>
					...
					<table class="table table-bordered table-hover table-condensed mb-none table-export nowrap">
						...
						<?php 
							...
							<tr>
								...
								<td><?php echo empty($row->created_at) ? '-' : _d($row->created_at);?></td>
								<td><a href="<?=base_url('addons/update/'.$row->prefix)?>" class="btn btn-default btn-circle"><i class="far fa-undo"></i> Update Check</a> </td>
							</tr>
							<?php endforeach; ?>
						</tbody>
					</table>
					...
					<footer class="panel-footer mt-lg">
						<div class="row">
							<div class="col-md-2 col-md-offset-3">
								<button type="submit" class="btn btn-default btn-block" name="submit" data-loading-text="<i class='far fa-spinner fa-spin'></i> Processing"><i class="far fa-file-import"></i> <?=translate('install_now')?></button>
							</div>
						</div>	
					</footer>
				<?php echo form_close();?>
