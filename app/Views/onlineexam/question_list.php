<section class="panel">
	<header class="panel-heading">
		<h4 class="panel-title"><i class="far fa-list-ul"></i> <?=$exam->title ." - ". translate('question') ." ". translate('list')?></h4>
		<div class="panel-btn">
			<a href="<?=base_url('onlineexam')?>" class="btn btn-default btn-circle">
				<i class="far fa-display"></i> <?=translate('exam') . " " . translate('list')?>
			</a>
		</div>
	</header>
	<div class="panel-body">
		<section class="panel pg-fw mt-lg">
		   <div class="panel-body">
		      	<h5 class="chart-title mb-xs"><i class="far fa-display"></i> <?=translate('online_exam') ." ". translate('details')?></h5>
		...
		<section class="panel pg-fw">
		   <div class="panel-body">
		      <h5 class="chart-title mb-xs"><i class="far fa-circle-question"></i> <?=translate('assign') ." ". translate('question')?></h5>
				<div class="table-responsive mt-lg">
					<table class="table table-bordered table-condensed table-hover">
						...
						<tbody>
							<?php 
								...
								$builder->select('class.name as class_name,subject.name as subject_name,section.name as section_name,questions.question,questions.type,questions.level,questions_manage.id,questions_manage.marks,questions_manage.neg_marks,question_group.name as group_name');
								$this->db->from('questions_manage');
								$builder->join('questions', 'questions.id = questions_manage.question_id', 'left');
								$builder->join('question_group', 'question_group.id = questions.group_id', 'left');
								$builder->join('class', 'class.id = questions.class_id', 'left');
								$builder->join('section', 'section.id = questions.section_id', 'left');
								$builder->join('subject', 'subject.id = questions.subject_id', 'left');
