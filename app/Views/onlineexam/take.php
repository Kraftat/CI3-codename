			...
			<table class="table table-striped table-condensed mb-none">
				<tbody>
					...
					<tr>
						...
						<td width="270"><?php echo $builder->select('transaction_id')->where(array('student_id' => get_loggedin_user_id(), 'exam_id' => $exam->id))->get('online_exam_payment')->row()->transaction_id; ?></td>
					</tr>
					<?php } ?>
				</tbody>
			</table>
...
if (empty($studentSubmitted)) {
	...
	?>
		<div class="text-center">
			<button class="btn btn-default btn-lg mt-lg start_btn" data-examid="<?php echo $exam->id; ?>" data-loading-text="<i class='far fa-spinner fa-spin'></i> Processing"><i class="far fa-computer-mouse"></i> <?=translate('start_exam')?></button>
		</div>
	<?php 
	}
         ...
         <div class="modal-dialog modal-dialogfullwidth">
            ...
            <div class="modal-content modal-contentfull">
               <div class="modal-header">
                  ...
                  <h4 class="modal-title"><i class="far fa-users-between-lines"></i> <?php echo $exam->title ?></h4>
               </div>
		        ...
		        if (data.status == 1) {
		            ...
		            $('#fueluxWizard').on('actionclicked.fu.wizard', function(e, data) {
		                ...
		                if (steps == totalQuestions) {
		                    $('#nextbutton i').remove();
		                    $('#nextbutton').append(' <i class="far fa-check"></i>');
		                    $('#finishedbutton').hide();
