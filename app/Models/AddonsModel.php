...
class AddonsModel extends MYModel
{
    protected $db;
    ...
    public function call_CurlApi($post_data)
    {
        ...
        if (!$url) {
            return (object) ['status' => false, 'message' => 'No internet connection.'];
        }
        ...
        if ($err !== '' && $err !== '0') {
            ...
            return (object) ['status' => false, 'message' => 'Unknown Error'];
        }
    }
    public function get_update_info($purchase_code)
    {
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS, ['item' => $purchase_code->prefix, 'current_version' => $purchase_code->version, 'system_version' => $this->get_current_db_version(), 'purchase_code' => $purchase_code->purchase_code]);
        $result = curl_exec($ch);
    ...
    {
        $builder->limit(1);
        return $db->table('addon')->get('addon')->row();
    }
}
